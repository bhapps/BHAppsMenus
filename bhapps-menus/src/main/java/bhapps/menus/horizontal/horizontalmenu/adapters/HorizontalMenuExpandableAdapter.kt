...
import bhapps.menus.horizontal.horizontalmenu.helpers.HorizontalMenuConfig.HORIZONTALMENU_ARROW_ROTATION_DURATION
import bhapps.menus.horizontal.horizontalmenu.helpers.HorizontalMenuConfig.HORIZONTALMENU_COLLAPSEOTHERS
import bhapps.menus.horizontal.horizontalmenu.helpers.HorizontalMenuConfig.HORIZONTALMENU_EXPANDCOLLAPSE_DELAY
import bhapps.menus.horizontal.horizontalmenu.helpers.HorizontalMenuConfig.HORIZONTALMENU_EXPANDCOLLAPSE_ROTATE
...
import bhapps.menus.horizontal.horizontalmenu.adapters.HorizontalMenuExpandableAdapter
import java.util.*

abstract class HorizontalMenuExpandableAdapter<T : HorizontalMenuItem>(
    protected var mContext: Context
) : RecyclerView.Adapter<HorizontalMenuExpandableAdapter<HorizontalMenuItem>.ViewHolder>() {
    ...
    open inner class HeaderViewHolder(
        ...
        protected fun handleClick() {
            ...
            Handler().postDelayed({ notifyDataSetChanged() },
                HORIZONTALMENU_EXPANDCOLLAPSE_DELAY.toLong()
            )
        }

        open fun bind(position: Int) {
            arrow.rotation =
                if (isExpanded(position)) HORIZONTALMENU_EXPANDCOLLAPSE_ROTATE.toFloat() else 0.toFloat()
        }
    ...
    fun collapseAllExcept(position: Int) {
        if (HORIZONTALMENU_COLLAPSEOTHERS) {
    ...
    companion object {
        ...
        val PARENT = HorizontalMenuType.PARENT.value
        val PARENTWITHCHILDREN = HorizontalMenuType.PARENTWITHCHILDREN.value
        val CHILD = HorizontalMenuType.CHILD.value
        val DIVIDER = HorizontalMenuType.DIVIDER.value
        val TITLE = HorizontalMenuType.TITLE.value

        private val ARROW_ROTATION_DURATION: Int = HORIZONTALMENU_ARROW_ROTATION_DURATION
